...
bool Tree::readSample(vector<pair<int, vector<double> > > all_samples) {
    ...
    if (root == NULL)
        root = new Node(numOfTypes);

    double p = 0.0;
    for (int i = 0; i < all_samples.size(); i++) {
        p = rand() * 1.0 / RAND_MAX;
        if (p <= ip.toDouble()) {  //每棵树用总样本的60%来训练,挑选的样本比例ip
...
bool Tree::buildTree() {
    ...
    //qDebug()<<"检验进程"<<endl;
    //qDebug()<<numOfFeatures<<numOfTypes<<endl;
    while (!q.empty()) {
       ...
       if (current->depth >= 25) //当当前节点的高度大于8时，当前节点不再进行分裂
            continue;

        if (current->sum < min_leaf_sample.toInt()) //当一个节点里样本数小于50时，该节点不再进行分裂，叶节点的最小样本数min_sample_leaf
            continue;
...
pair<Node*, Node*> Tree::selectBestPart(Node* current) {
    ...
    Node* small = new Node(numOfTypes);
    Node* large = new Node(numOfTypes);
